#!/usr/bin/env bash
# Phase 2: SessionStart hook behavior — requires gcloud CLI.
...
# T2.1 — hook succeeds with no output when gcloud is installed
test_hook_succeeds_when_gcloud_installed() {
...
# T2.2 — hook warns when gcloud is not on PATH
test_hook_warns_when_gcloud_missing() {
  local cmd
  cmd=$(_get_hook_command)
  local tmp
  tmp=$(mktemp -d)
